...
async function main() {
  ...
  if (viewer) {
    console.log(`       ${viewer.organization.name} (${viewer.organization.urlKey}) as ${viewer.viewer.name}`);
  }

  const issues = viewer
    ? await step("open issues assigned to you, with relations", () =>
        client.request<{
          issues: { nodes: Parameters<typeof deriveRelations>[0][]; pageInfo: { hasNextPage: boolean } };
  ...
  const projects = await step("active projects with targets", () =>
    client.request<{
      projects: {
        nodes: Array<{ id: string; name: string; targetDate: string | null; progress: number | null }>;
  ...
  if (projects?.projects.nodes.length) {
    ...
    await step(`project scope for "${first.name}"`, async () => {
      const data = await client.request<{
        project: { issues: { nodes: Array<{ estimate: number | null; state: { type: string } }> } };
      }>(PROJECT_SCOPE, { projectId: first.id, after: null });
      console.log(`       scope estimate: ${sumScope(data.project.issues.nodes) ?? "none (falls back to neutral)"}`);
      return data;
    });
  }
  ...
  if (issues && projects) {
    ...
    const withTarget = nodes.filter(
      (n) => (n as { project?: { id: string } | null }).project?.id && targeted.has((n as { project: { id: string } }).project.id),
    ).length;
    const blocked = nodes.filter((n) => deriveRelations(n).isBlocked).length;

    console.log("\nRanking readiness");
    console.log(`  open issues assigned to you   ${nodes.length}${issues.issues.pageInfo.hasNextPage ? "+ (more pages)" : ""}`);
    console.log(`  blocked, so excluded          ${blocked}`);
    console.log(`  active projects               ${projects.projects.nodes.length}`);
